...
class CustomSalarySlip(TransactionBase):
	"""
	Custom Salary Slip controller that extends standard ERPNext SalarySlip
	with attendance-based salary calculation while preserving all tax functionality
	"""
	...
	def validate(self):
		"""
		Custom validate method that integrates attendance-based salary calculation
		while preserving the standard ERPNext salary slip validation flow
		"""
		...
		# Continue with standard validation flow first
		...
		# CUSTOM LOGIC: Check if attendance-based calculation is needed
		if self.should_calculate_from_attendance():
			# Ensure salary structure doc is loaded for attendance-based calculations
			if not hasattr(self, '_salary_structure_doc') or not self._salary_structure_doc:
				self.set_salary_structure_doc()
			# For attendance-based employees, override specific components after salary structure is loaded
			self.calculate_attendance_based_salary()
			# Set flag to indicate custom calculation was done
		...
		# CUSTOM LOGIC: For regular employees, handle advances and basic salary as needed
		if not self.should_calculate_from_attendance():
			if self.salary_structure:
				# Salary structure exists, just handle advances (replace any existing Advances component)
				self.handle_employee_advances()
			else:
				# No salary structure, add basic salary and advances
				self.add_basic_salary_component()

		# Ensure tax components are refreshed for UI display
		# This ensures the deductions table shows tax components immediately
		if self.salary_structure and self.payroll_period:
	...
	def on_submit(self):
		"""
		Custom on_submit that includes advance deduction linking
		"""
		if self.net_pay < 0:
		...
		# CUSTOM LOGIC: Link advance deductions for attendance-based employees
		if self.should_calculate_from_attendance():
	...
	def on_cancel(self):
		"""
		Custom on_cancel that includes advance deduction unlinking
		"""
		self.set_status()
		self.update_status()
		self.update_payment_status_for_gratuity_and_leave_encashment()
		...
		# CUSTOM LOGIC: Unlink advance deductions for attendance-based employees
		if self.should_calculate_from_attendance():
	...
	def calculate_net_pay(self, skip_tax_breakup_computation: bool = False):
		...
		# Skip component recalculation for attendance-based employees
		if self.salary_structure and not getattr(self, '_custom_calculation_done', False):
	...
	def compute_taxable_earnings_for_year(self):
		...
		# Debug logging for annual taxable earnings
		frappe.log_error(
			message=f"Annual taxable breakdown: previous={self.previous_taxable_earnings}, "
					f"current_structured={self.current_structured_taxable_earnings}, "
					f"future_structured={self.future_structured_taxable_earnings}, "
					f"additional={self.current_additional_earnings}, "
					f"other_incomes={self.other_incomes}, "
					f"unclaimed_benefits={self.unclaimed_taxable_benefits}, "
					f"exemptions={self.total_exemption_amount}, "
					f"TOTAL_ANNUAL={self.total_taxable_earnings}",
			title="Annual Taxable Earnings"
		)
	...
	def compute_current_and_future_taxable_earnings(self):
		...
		# Use remaining months in tax year for future earnings projection, not payroll period
		remaining_tax_periods = self.get_remaining_tax_periods()
		self.future_structured_taxable_earnings = self.current_taxable_earnings.taxable_earnings * remaining_tax_periods

		# Debug logging
		frappe.log_error(
			message=f"Tax calculation: current_taxable={self.current_taxable_earnings.taxable_earnings}, "
					f"remaining_tax_periods={remaining_tax_periods}, "
					f"future_structured={self.future_structured_taxable_earnings}, "
					f"payroll_remaining={self.remaining_sub_periods}",
			title="Tax Period Calculation"
		)
		...
		self.future_structured_taxable_earnings_before_exemption = (
			current_taxable_earnings_before_exemption * remaining_tax_periods
		)
	...
	def add_tax_components(self):
		...
		frappe.log_error(message=f"tax_components: {tax_components}", title="Tax Components")
		...
		if tax_components and self.payroll_period and self.salary_structure:
			...
			frappe.log_error(message=f"tax_slab: {self.tax_slab}", title="Tax Slab")
	...
	def get_amount_based_on_payment_days(self, row):
		...
		# For attendance-based calculations, don't apply payment day adjustments to our custom components
		if getattr(self, '_custom_calculation_done', False) and row.salary_component in ['Gross Salary', 'Overtime', 'Overtime GZT', 'Deficiency', 'Advances']:
			return amount, additional_amount

		timesheet_component = None
		if hasattr(self, '_salary_structure_doc') and self._salary_structure_doc:
		...
		if (
			...
			and (
				(timesheet_component is None or row.salary_component != timesheet_component)
			...
			and (timesheet_component is None or row.salary_component != timesheet_component)
			and cint(row.depends_on_payment_days)
		):
	...
	def get_remaining_tax_periods(self):
		"""
		Calculate remaining months in the tax year from current salary slip end date
		This is used for tax calculation projection, not payroll period calculation
		"""
		# Get fiscal year (tax year)
		fiscal_year = get_fiscal_year(date=self.start_date, company=self.company, as_dict=1)
		tax_year_start = getdate(fiscal_year.year_start_date)
		tax_year_end = getdate(fiscal_year.year_end_date)

		# Current salary slip period
		current_end = getdate(self.end_date)

		# If current salary slip is beyond tax year end, return 0
		if current_end >= tax_year_end:
			return 0

		# Calculate remaining months from end of current salary slip to end of tax year
		remaining_months = (tax_year_end.year - current_end.year) * 12 + (tax_year_end.month - current_end.month)

		# If we're in the same month, we need to check if the salary slip covers the whole month
		if remaining_months == 0:
			# If salary slip end date is before month end, there might be remaining days in the month
			# But for tax calculation, we typically project by full months
			return 0
	...
	# CUSTOM METHODS FROM salary_slip_controller.py
	# These methods implement attendance-based salary calculation
	...
	def calculate_attendance_based_salary(self):
		...
		Override specific components in existing salary structure with calculated values
		"""
		...
		# Update existing components with our calculated values instead of clearing everything

		# Update Gross Salary component
		self.update_or_add_component('earnings', 'Gross Salary', gross_salary)

		# Update Overtime components
		self.update_or_add_component('earnings', 'Overtime', overtime_amount)
		self.update_or_add_component('earnings', 'Overtime GZT', gzt_overtime_amount)

		# Update Deficiency component
		self.update_or_add_component('deductions', 'Deficiency', deficiency_amount)

		# Update Advances component (remove existing and add calculated)
		if hasattr(self, 'deductions') and self.deductions:
			self.deductions = [d for d in self.deductions if d.get('salary_component') != 'Advances']

		if advances_amount > 0:
			self.append('deductions', {
				'salary_component': 'Advances',
				'amount': advances_amount,
				'exempted_from_income_tax': 1,  # Advances should not reduce taxable income
				'is_tax_applicable': 0,  # Advances themselves are not taxable
		...
		# Debug logging
		frappe.log_error(
			message=f"Attendance calculation completed: gross={gross_salary}, overtime={overtime_amount}, "
					f"gzt_overtime={gzt_overtime_amount}, deficiency={deficiency_amount}, advances={advances_amount}",
			title="Attendance Salary Calculation"
		)
	...
	def update_or_add_component(self, component_type, component_name, amount):
		"""
		Update an existing component amount or add it if it doesn't exist
		Used for attendance-based salary calculations
		"""
		if amount <= 0:
			# Remove component if amount is 0 or negative
			if hasattr(self, component_type) and self.get(component_type):
				setattr(self, component_type, [
					d for d in self.get(component_type)
					if d.get('salary_component') != component_name
				])
			return
		...
		for component in self.get(component_type):
			if component.get('salary_component') == component_name:
				existing_component = component
				break

		if existing_component:
			# Update existing component amount
			existing_component.amount = amount
			existing_component.default_amount = amount
			# Ensure tax attributes are correct for attendance-based calculations
			if component_type == 'earnings' and component_name in ['Gross Salary', 'Overtime', 'Overtime GZT']:
				existing_component.is_tax_applicable = 1
			elif component_type == 'deductions' and component_name == 'Deficiency':
				existing_component.is_tax_applicable = 0
				existing_component.exempted_from_income_tax = 0
		else:
			# Add new component - get component data from salary component master
			component_data = get_salary_component_data(component_name)
			if component_data:
				...
				# Fallback if component not found in master
				component_data = {
					'salary_component': component_name,
					'abbr': component_name.replace(' ', '').upper()[:5],
					'amount': amount,
					'default_amount': amount,
					'additional_amount': 0,
					'is_tax_applicable': 1 if component_type == 'earnings' else 0,
					'depends_on_payment_days': 0,
					'do_not_include_in_total': 0,
					'is_flexible_benefit': 0,
					'variable_based_on_taxable_salary': 0,
				...
				# Special handling for deduction components
				if component_type == 'deductions':
					if component_name == 'Deficiency':
						component_data.update({
							'is_tax_applicable': 0,
							'exempted_from_income_tax': 0
						})

			# Ensure standard earning components are tax applicable
			if component_type == 'earnings' and component_name in ['Gross Salary', 'Overtime', 'Overtime GZT']:
				component_data['is_tax_applicable'] = 1

			self.append(component_type, component_data)

	def handle_employee_advances(self):
		"""
		Handle employee advances for regular employees by removing any existing Advances
		component from salary structure and adding the calculated advances amount
		"""
		# Check for employee advances and add as deductions
		advances_data = self.get_employee_advances_with_ids()
		advances_amount = advances_data['total_amount']

		# Remove any existing Advances component that might have been loaded from salary structure
		if hasattr(self, 'deductions') and self.deductions:
			self.deductions = [d for d in self.deductions if d.get('salary_component') != 'Advances']

		# Add our calculated advances component
		if advances_amount > 0:
			self.append('deductions', {
				'salary_component': 'Advances',
				'amount': advances_amount,
	...
	def add_basic_salary_component(self):
		...
		# Also handle advances for this case
		self.handle_employee_advances()
